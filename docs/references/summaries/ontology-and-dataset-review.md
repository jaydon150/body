...
- **OpenAnatomy atlas-page-level license inheritance = 3D Slicer License (BSD-style, permissive)** — verified directly at the canonical project page `https://www.openanatomy.org/atlas-pages/slicer-license.html` and at a per-atlas LICENSE file `https://github.com/mhalle/spl-brain-atlas/blob/master/LICENSE.md` (both retrieved 2026-05-11). Three-part agreement (Part A contributions / Part B downloads / Part C miscellaneous) — Part B applies to atlas downloads. **No share-alike obligation.** Commercial use permitted "at the sole risk" of the commercializing party. Attribution required: licensees must preserve all applicable attributions and copyright notices, and include the license terms with any redistribution. **Clinical applications explicitly discouraged.** Massachusetts law governs. **The research report's overall stance on OpenAnatomy was directionally correct; its "Section B" gloss was sloppy** (Part B is a general software-downloads agreement, not an anatomy-specific section), but the substance — that atlases inherit a permissive Slicer license that allows commercial redistribution with attribution — holds up. ADR 0005's OpenAnatomy slot is solid.
